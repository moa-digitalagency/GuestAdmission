...
class StatisticsService:
    """Service pour générer des statistiques détaillées"""
    ...
    def get_top_countries(etablissement_id: Optional[int] = None, limit: int = 10) -> List[Dict]:
        """Récupérer les pays d'origine les plus fréquents"""
        conn = get_db_connection()
        cur = conn.cursor()
    ...
    def get_sejours_by_occupants(etablissement_id: Optional[int] = None, limit: int = 10) -> List[Dict]:
        """Récupérer les séjours avec le plus grand nombre d'occupants"""
        conn = get_db_connection()
        cur = conn.cursor()
